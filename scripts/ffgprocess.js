...
let translations = JSON.parse(fs.readFileSync('../data/translations/translations.json', 'utf8'));
let upgradeTypes = JSON.parse(fs.readFileSync('../data/upgrade-types/upgrade-types.json', 'utf8'));
...
function processCard(card) {
    ...
    if (card.card_type_id == 1) {
        Object.entries(pilotData).forEach(
            ...
            ([ filenameKey, ship ]) => {
                ...
                if (ship.ffg == card.ship_type && ship.faction == getFaction(card.faction_id)) {
                    // Save the filename
    ...
    if (ship_ability_text) {
        ...
        ship_ability.name = getTagContent(ship_ability_text, "sabold").replace(/\:/g, "").trim();
...
if (unsanitized && unsanitized.length) {
    ...
    if (unhandledTags.length) {    
        console.log("** Please add unhandled tags to ../translations/translations.json and re-run **");
        unhandledTags.forEach(
            (tag) => {
                console.log(tag);
            }
        )
...
extractFileList(manifest["pilots"]).forEach(
    (file) => {
...
extractFileList(manifest["upgrades"]).forEach(
    (file) => {
...
if (modifiedFiles.length) {
    ...
    modifiedFiles.forEach(
        (filename) => {
            ...
            let data = pilotData[filename] ? pilotData[filename] : upgradeData[filename];
